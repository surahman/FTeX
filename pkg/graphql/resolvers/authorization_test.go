...
func TestGinContextFromContext(t *testing.T) {
	...
	testCases := []struct {
	...
	}{
		{
			...
			expectErr:   require.Error,
			ctx:         context.WithValue(t.Context(), GinContextKey{}, t.Context()),
		}, {
			name:      "success",
			expectErr: require.NoError,
			ctx:       context.WithValue(t.Context(), GinContextKey{}, &gin.Context{}),
		},
	}
...
func TestAuthorizationCheck(t *testing.T) {
	...
	testCases := []struct {
	...
	}{
		{
			...
			expectErr:            require.Error,
			ctx:                  context.WithValue(t.Context(), GinContextKey{}, ginCtxNoAuth),
			authValidateJWTErr:   nil,
			authValidateJWTTimes: 0,
			isDeletedError:       nil,
			isDeletedTimes:       0,
			...
			expectErr:            require.Error,
			ctx:                  context.WithValue(t.Context(), GinContextKey{}, ginCtxAuth),
